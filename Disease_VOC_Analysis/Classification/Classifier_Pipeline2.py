...
DIR='/Users/joshswore/Manduca/MultiChannel/Floral/Quality_Controlled_Data/ButterLC.1_HC6/YY_Normalized_Smoothened/CH_Addition/'
...
for f in DirList:
    if f.endswith('.csv'):
        ...
        CH1=All_df.iloc[:,:-3]
        Ch_List=[CH1]#,CH2]
        #Ch_Name_list=['CH1','CH2']

        for ch in Ch_List:
            PCA_DF, PCA_M, Scaled_Data = PCA_Constructor(CH1,10)
            PCA_DF=pd.concat([PCA_DF,All_df.iloc[:,-3:]],axis=1)
            PCA_Results=[PCA_DF,PCA_M, Scaled_Data]
            Dlabs=['PCA_DF','PCA_M','Scaled_Data']
            n = f.split('.csv')[0]
            Save_Directory = f'{DIR}ClassifierResults/10PCs/{n}/'
            for d,l in zip(PCA_Results,Dlabs):
                pickle_Saver(Save_Directory,f'{l}',d)
...
DIR='/Users/joshswore/Manduca/MultiChannel/Floral/Quality_Controlled_Data/ButterLC.1_HC6/YY_Normalized_Smoothened/CH_Subtraction/'
DirList = os.listdir(DIR)
...
DIR='/Users/joshswore/Manduca/MultiChannel/Floral/Quality_Controlled_Data/ButterLC.1_HC6/YY_Normalized_Smoothened/Both_Channels/'
DirList = os.listdir(DIR)
print(f'beginng Classification of YY_Normalized channels subtracted Data')
for f in DirList:
    if f.endswith('.csv'):
        ...
        for ch,chn in zip(Ch_List,Ch_Name_list):
            ...
            RF_results=RFmodel(concentrations='1k',data=[PCA_DF],odor='linalool|limonene|lemonoil',PosL='lemonoil', repeats=100)
            pickle_Saver(savedir=Save_Directory,ext='RF_Results',data=RF_results)
DIR='/Users/joshswore/Manduca/MultiChannel/Floral/Quality_Controlled_Data/ButterLC.1_HC6/YY_Normalized/CH_Addition/'
DirList = os.listdir(DIR)
print(f'beginng Classification of YY_Normalized channels summed Data')
for f in DirList:
    if f.endswith('.csv'):
...
DIR='/Users/joshswore/Manduca/MultiChannel/Floral/Quality_Controlled_Data/ButterLC.1_HC6/YY_Normalized/CH_Subtraction/'
DirList = os.listdir(DIR)
print(f'beginng Classification of YY_Normalized channels subtracted Data')
for f in DirList:
    if f.endswith('.csv'):
...
DIR='/Users/joshswore/Manduca/MultiChannel/Floral/Quality_Controlled_Data/ButterLC.1_HC6/YY_Normalized/Both_Channels/'
DirList = os.listdir(DIR)
print(f'beginng Classification of YY_Normalized channels subtracted Data')
for f in DirList:
    if f.endswith('.csv'):
